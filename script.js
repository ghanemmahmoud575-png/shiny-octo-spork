...
/* ========= Helpers ========= */
const f2 = v => Number(v || 0).toFixed(2);     // app → 2 decimals
const r0 = v => Math.round(Number(v || 0));   // whatsapp → integer
...
function deleteTruck(i){
  if(!confirm("⚠️ هل أنت متأكد من حذف هذه التركه؟")) return;
  trucks.splice(i,1);
  isSaved = false;
  renderTrucks();
  liveCalc();
...
function renderTrucks(){
  ...
  trucks.forEach((t,i)=>{
    ...
    list.innerHTML += `
      <div class="card">
        <b>🚚 Truck ${i+1}</b> – unloading time: ${minutes} min

        <div class="row">
          <div><label>Driver (السائق) </label><input value="${t.driver}" oninput="trucks[${i}].driver=this.value"></div>
          <div><label>Date (تاريخ التفريغ) </label><input value="${t.date}" oninput="trucks[${i}].date=this.value"></div>
        </div>

        <div class="row">
          <div><label>Station (المحطه) </label><input value="${t.station}" oninput="trucks[${i}].station=this.value"></div>
          <div><label>Quantity (الكميه ) (BBL)</label><input type="number" value="${t.qty}" oninput="trucks[${i}].qty=this.value;liveCalc()"></div>
        </div>

        <div class="row">
          <div><label>Start Time (بداية وقت التفريغ) </label><input type="time" value="${t.start}" oninput="trucks[${i}].start=this.value;renderTrucks()"></div>
          <div><label>End Time (نهاية وقت التفريغ) </label><input type="time" value="${t.end}" oninput="trucks[${i}].end=this.value;renderTrucks()"></div>
        </div>

        <button onclick="deleteTruck(${i})">🗑 Delete</button>
      </div>
    `;
  });
}
  ...
  Object.keys(savedDays).forEach(d=>{
    savedDaysList.innerHTML += `
      <div class="saved-item">
        <span>${d}</span>
        <button onclick="editDay('${d}')">✏️</button>
        <button onclick="sendSavedWhatsApp('${d}')">📲</button>
        <button onclick="deleteDay('${d}')">🗑</button>
      </div>
    `;
  });
}
...
function sendWhatsApp(){
  ...
  msg += `》Shipped ${trucks.length} truck(s) from\n${stations}\n\n`;

  trucks.forEach(t=>{
    msg += `》Ticket no\n`;
    msg += `\t● shipped @ ${formatDayMonth(t.date)}\n`;
    msg += `\t● shipped volume : ${r0(t.qty)} bbl\n`;
    msg += `\t● unloading running time: ${r0(calcMinutes(t.start,t.end))} min\n`;
  });

  msg += `\n》Received in TK(${tankSelect.value})\n`;
  msg += `\t● Last close for the tank = ${r0(beforeCC.value)} cm = ${r0(cmToBbl(beforeCC.value))} bbl\n\n`;

  msg += `》After unloading in TK\n`;
  msg += `\t● Gross level = ${r0(afterCC.value)} cm = ${r0(cmToBbl(afterCC.value))} bbl\n`;
  msg += `( ${r0(Math.abs(Number(actualTankNet.innerText)))} bbl ${status.innerText.toLowerCase()} than sent )\n\n`;

  if(afterDrainCC.value){
    msg += `\t● Gross level after drain = ${r0(afterDrainCC.value)} cm = ${r0(cmToBbl(afterDrainCC.value))} bbl\n`;
    msg += `\t( ${r0(drainQty.innerText)} bbl drain free water )\n\n`;
  }

  msg += `》Taking samples to DAPETCO lab.\n\t● BS&W :  % \n\t● salt content :  PTB \n\t● API :  ° \n\n`;
  msg += `》Shipping to El-hamra\n`;
  msg += `\t● Close level = ${r0(afterHamraCC.value)} cm = ${r0(cmToBbl(afterHamraCC.value))} bbl`;
